...
## 5. Fix Reclass Expansion Bug in getClassChangeOptions

- [x] 5.1 Fix `getClassChangeOptions` in `lib/stats.ts` to show ALL reclass-reachable classes without level/tier validation. Replace the `getValidReclassOptions()` call (lines 365-372) with direct expansion: iterate `unit.reclassOptions`, add each base class, then walk each base class's `promotesTo` chain and add those too. Do NOT filter through `isValidReclass`. The display is informational — it shows every class the unit could ever reach through any combination of reclass + promotion, regardless of current level or tier. Preserve the game gate (`unit.game?.toLowerCase() === 'awakening'`). Exclude the unit's base class. Deduplicate by class ID.
      **Done when**: Chrom (level 1) shows all reclass classes (Cavalier, Archer, Paladin, Great Knight, Sniper, Bow Knight) plus Great Lord. Cherche (level 12) shows Troubadour, Cleric, and all their promoted forms in addition to Wyvern Rider promotions. Robin (level 1) shows all reclass options and their promoted forms. Verify by running the dev server and comparing the class change options row against the full expected class list for at least these three units. `npm run build` passes.

## 6. Remove PromotionOptionsDisplay

- [x] 6.1 Remove the `PromotionOptionsDisplay` render block from `app/comparator/page.tsx` (lines 71-77, the `<PromotionOptionsDisplay key={...} unit={unit} />` calls). Remove the import of `PromotionOptionsDisplay` from the same file. Do NOT delete the `PromotionOptionsDisplay.tsx` file itself (leave for follow-up cleanup).
