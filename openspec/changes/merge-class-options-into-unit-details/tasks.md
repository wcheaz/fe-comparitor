...
- [x] 7.1 Remove the `PromotionOptionsDisplay` render block from `app/comparator/page.tsx` (lines 71-77, the `<PromotionOptionsDisplay key={...} unit={unit} />` calls). Remove the import of `PromotionOptionsDisplay` from the same file. Do NOT delete the `PromotionOptionsDisplay.tsx` file itself (leave for follow-up cleanup).
