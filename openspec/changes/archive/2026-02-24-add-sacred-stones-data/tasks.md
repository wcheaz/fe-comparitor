...
- [x] 3.4 Update the `generateProgressionArray` mapping logic in the table rendering code to correctly handle the new `generateProgressionArray` function signature and properly label row headers (e.g., "Level 1 (Tier 2)").
...
- [x] 4.3 Write a test script to validate average stat generation accuracy for a 3-tier branching unit (e.g., Ross, Amelia, Ewan), verifying cumulative stats across all 3 class stages.
- [x] 4.4 Run `openspec archive change add-sacred-stones-data` once all tasks are complete and verified.
