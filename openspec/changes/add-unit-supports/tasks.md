...
- [x] Inside the `<Modal>` content for supports, calculate the combined C, B, A, and S rank bonuses using `lib/affinities.ts` taking both units' affinities into account (if the game supports affinity math, primarily GBA and PoR).
