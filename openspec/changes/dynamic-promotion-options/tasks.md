...
## 10. Fix Unscrollable Body on Nested Modal Close

- [x] 10.1 Diagnose `components/ui/modal.tsx` (or whatever hook manages body scrolling when modals open/close). The issue is likely that closing the inner modal removes `overflow: hidden` from the body, or closing the outer modal fails to restore `overflow: auto`/`unset` because of conflicting state or race conditions.
- [x] 10.2 Consider implementing a global or contextual reference counter (e.g., a simple module-level `let openModalCount = 0;`) inside the Modal/portal logic to track how many modals are currently active. Increment on mount/open, decrement on unmount/close.
